...
const Subtraction = () => {
  ...
  const handleCalculate = () => {
    const i = (Number(X1) || 0) - (Number(X2) || 0);
    const j = (Number(Y1) || 0) - (Number(Y2) || 0);
    const k = (Number(Z1) || 0) - (Number(Z2) || 0);

    const terms = [];
    if (i !== 0) terms.push(`${i > 0 ? "" : ""}${i}i`);
    if (j !== 0) terms.push(`${j > 0 ? " + " : " - "}${Math.abs(j)}j`);
    if (k !== 0) terms.push(`${k > 0 ? " + " : " - "}${Math.abs(k)}k`);

    const result = terms.length > 0 ? terms.join("") : "0";
    setAns(result || "0");
  };
